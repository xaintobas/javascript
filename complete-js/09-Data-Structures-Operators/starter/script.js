...
document.querySelector('button').addEventListener('click', function () {
  ...
  for(const [i, row] of rows.entries()){
    const trimWords = row.trim().toLowerCase();
    const [firstWord, secondWord] = trimWords.split('_');
    const output = `${firstWord}${secondWord.replace(secondWord[0], secondWord[0].toUpperCase())}`;
    const padWord = output.padEnd(20);
    const repeatedOk = '✅'.repeat(i + 1);
    console.log(padWord, repeatedOk);
  }
});
